...
class Solution {
    ...
    // we only use i-1
    // NOTICE: compute dp[j][k], we need to use dp[j-group[i]][k - profit[i]], which index is smaller, so we need to loop from larger value.
    ...
    public int profitableSchemes(int G, int P, int[] group, int[] profit) {
        ...
        for (int i = 1; i < group.length; i++) for (int j = G; j >= group[i]; j--) for (int k = P; k >= 0; k--) {        
            // dp[j][k] = dp[j][k]; // not add i-th

            // add i-th, notic the overflow
            if (j >= group[i]) {
                dp[j][k] = (dp[j][k] + dp[j - group[i]][Math.max(0, k - profit[i])]) % modulo;
            }
        }

        long res = 0;
        for(int i = 0; i <= G; i++) res = (res + dp[i][P]) % modulo;
        return (int)res;
    }
}
// @lc code=end
